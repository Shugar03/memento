# Multi-Agent Same-Tenant â€” End-to-End Validation
...
- `workspace_id` is **deterministic per tenant** (`SHA256(tenant_id || "memento-default-workspace")` first 16 bytes, see `crates/memento-tenant/src/resolver.rs:58-67`). Two agents on the same tenant therefore land on the **same default workspace** by construction â€” that is why they share data without any extra wiring.
...
|---|---|
| `crates/memento-domain/src/tenant.rs:21-39` | `TenantContext { tenant_id, workspace_id, agent_id }` â€” all three bound at startup, `pub(crate)` constructor under `tenant-resolver` feature, opaque to every other crate. |
| `crates/memento-tenant/src/resolver.rs:58-67` | `default_workspace_id(tenant_id)` = `SHA256(tenant_id_bytes â€– "memento-default-workspace")` first 16 bytes â€” **deterministic per tenant, stable across restarts**. |
...
| `crates/memento-lancedb/src/schema.rs:176-178` | `chunks_scope = tenant_scope AND workspace_scope` â€” the FTS/vector hybrid filter. **`agent_id` is not in the filter.** |
...
| `crates/memento-lancedb/src/feedback.rs:88-110` | Feedback reads scoped by `tenant_id + chunk_id` only â€” but feedback *writes* store the bound `agent_id` as a column (`feedback.rs:31-58` builder). |
| `crates/memento-lancedb/src/schema.rs:43,98,118,132` | `agent_id` is a column on `chunks`, `docs`, `feedback`, but **never** a query filter. |

**Consequence**: agent_id is provenance/attribution only. The isolation boundary that prevents data leakage is `workspace_id`. Two agents on the same tenant that share a workspace *must* see each other's writes â€” by construction, not by accident.
...
```
$ memento tenant create --name "multi-agent-test" --root "<STORAGE_ROOT>" --json
{
...
### Phase 2 â€” Same-workspace shared (Agent A writes, Agent B reads)

**Agent A (`codex-agent`) ingests:**

```
$ memento ingest text "Mensaje del agente Codex: lo dejÃ© como sugerencia en el archivo main.rs." --json
...
{"hits":[{
  ...
  "text":"Mensaje del agente Codex: lo dejÃ© como sugerencia en el archivo main.rs.",
  "provenance":{
...
### Phase 3 â€” Different-workspace isolation (search-side)
...
### Phase 4 â€” Same workspace, cross-write visibility (Agent B writes, Agent A reads)
...
### Phase 5 â€” Code projects shared

```
$ MEMENTO_AGENT_ID=codex-agent memento code index "<PROJECT_ROOT>\crates\memento-domain\src" --json
...
### Phase 6 â€” Feedback attribution
...
### Phase 7 â€” Tenant delete covers all agents
...
| Non-default workspaces are writable from the CLI (`ingest --workspace`) | **FAIL â€” gap (see below)** |
| Workspace can be created from the CLI (`workspace create` / list) | **FAIL â€” gap (see below)** |
| `stats` exposes per-agent breakdown | **FAIL â€” gap (see below)** |

---

## Gaps (non-blocking, document-and-decide-later)

### Gap 1 â€” Write isolation is not reachable from the CLI
...
**Severity**: medium. The design intent (workspace as isolation boundary) is honored by the read path; the write path is locked to the default workspace. There is no data leakage â€” there is just no way to *use* the boundary for writes from the CLI yet.
...
### Gap 2 â€” No `workspace` subcommand
...
### Gap 3 â€” No `chunks_by_agent` in `stats`
...
1. **Two agents sharing a tenant is not just supported â€” it is the default behavior.** Anyone running Memento RS with a single token and two different `MEMENTO_AGENT_ID` values already has a multi-agent store, with full write/read cross-visibility and clean provenance attribution. No additional wiring is required.
2. **`workspace_id` is the real isolation boundary**, as the design said. To isolate two agents on the same tenant from each other, give them different workspaces â€” but only the read side of the CLI can target a non-default workspace today (Gap 1). The MCP surface can target any workspace for writes; the CLI cannot.
...
- `crates/memento-domain/src/tenant.rs:21-39` â€” `TenantContext` shape
- `crates/memento-tenant/src/resolver.rs:58-67` â€” `default_workspace_id` derivation
- `crates/memento-lancedb/src/schema.rs:176-178` â€” `chunks_scope` (tenant + workspace only)
- `crates/memento-lancedb/src/fts.rs:96-108` â€” FTS scope expression
- `crates/memento-lancedb/src/feedback.rs:31-58` â€” feedback row builder (per-agent attribution)
- `crates/memento-cli/src/commands/memory.rs:97-114` â€” `feedback` CLI handler
- `crates/memento-cli/src/commands/memory.rs:116-159` â€” `delete` handler (`--workspace` accepted)
- `crates/memento-cli/src/commands/memory.rs:214-222` â€” `workspace_of` helper (read-only flag)
- `docs/validation/gdpr-right-to-erase-e2e.md` â€” the erase chain this validation builds on