# GDPR Right-to-Erase — End-to-End Validation
...
**Verdict**: **PASS** — the right-to-erase is real and complete; one **non-blocking design choice** documented.
...
- The purge chain (`delete → Compact → Prune` per `memento_lancedb::erase`) ran on every LanceDB table. The unique fingerprint `ZQXWFINGERPRINT7788` and the string `DNI 12345678` are **GONE** from the entire storage root (binary `mmap.find` over every file: `0` hits before/after the new tenant was created).
...
- The old token returns `AUTH_FAILED` immediately after the ceremony (the credential hash is gone — no signature, no reuse possible).
...
```
$ memento tenant create --name "gdpr-victim" --root "F:\.memento-smoke" --json
{
  "name": "gdpr-victim",
  "tenant_id": "019fee80-4b91-7ae2-a4b6-65839314ee1a",
  "token": "memo_019fee80-4b91-7ae2-a4b6-65839314ee1a_LtwWIB5lsa9RvTkKgJsM1YVpd9wFv3jgbbgs6JqiHDOcQh2G"
}
```

`token` printed exactly once; only the Argon2id PHC hash is persisted (REQ-TA-006).

### Phase 2 — Data ingestion (representative tenant data)
...
| Ingest PDF (Hypnotic Writing, copied inside storage root because of the path-jail guard) | `memento --no-embeddings ingest document --source 'document:pdf' "F:\.memento-smoke\inbox\hw.pdf"` | 44 chunks, doc `019fee80-ed37-72b0-a059-dcf4ca85624b` |
| Code index (Memento RS `src/`) | `memento code index "F:\OPENCODE proyectos\Memento RS\src"` | Project `3b992aa72571d100` (1 module, 1 symbol, 1 node) |
...
### Phase 3 — Pre-delete baseline snapshot

| Property | Value |
|---|---|
| Tenant directory | `F:\.memento-smoke\db\tenants\019fee80-...` |
| File count | **47** |
| Total size | **116,136 bytes** (~113 KiB) |
| LanceDB size | 105,815 bytes |
| Audit log lines | 2 (`ingest` × 2) |
| Backups | 0 |
...
### Phase 4 — Confirmation ceremony

Without confirmation → **aborts**:

```
$ echo "no" | memento tenant delete --root "F:\.memento-smoke" --json
{"code":"INVALID_INPUT","detail":"invalid input: deletion aborted: type 'yes' to confirm","exit_code":2}
exit_code = 2
```

With `yes` on stdin → **proceeds**:

```
$ echo "yes" | memento tenant delete --root "F:\.memento-smoke" --json
{
  "backups_count": 0,
...
### Phase 5 — Post-delete state

| Property | Value | Verdict |
|---|---|---|
| Tenant directory exists | `True` | ⚠️ design choice (see below) |
...
| `okf-bundles/3b992aa72571d100/` | **gone** (code indexes purged) | PASS — REQ-CG-001 |
...
| Old token `memento search "DNI" --json` | `{"code":"AUTH_FAILED", "exit_code":4}` | PASS — token no longer signs |
...
### Phase 6 — Recreate same name

```
$ memento tenant create --name "gdpr-victim" --root "F:\.memento-smoke" --json
{
  "name": "gdpr-victim",
  "tenant_id": "019fee84-5e95-7df2-8ae5-1b3868669a5d",  ← NEW id
  "token": "memo_019fee84-..._95L7fNTcIolXTzj0kwuLAZ7rJLQKKTi2uS0i2cs7USYS7fwJ"
}
```
...
```
F:\.memento-smoke\db\tenants\019fee84-5e95-7df2-8ae5-1b3868669a5d\
├── auth/
│   └── credentials.toml
└── config.toml
```

The new tenant's `search "ZQXWFINGERPRINT7788" --json` returns `{"hits":[]}` — **no carry-over of any old data**.

Ingesting "tenant-2 clean test" into the new tenant works normally, and a **final** binary `mmap.find` over the **entire** `F:\.memento-smoke` root returns `0` hits for `ZQXWFINGERPRINT7788` — confirming the old secret is gone, not just hidden behind auth.

### Phase 7 — Non-existent tenant
...
> The tenant dir itself stays — on Windows the open LanceDB store holds file locks; the store was already purged+pruned.

**What remains on disk after erase**:

- `lancedb/chunks.lance/_transactions/`, `_versions/`, `data/`, `_indices/` — empty table shells (no row data, no FTS terms, no embeddings)
- `lancedb/docs.lance/`, `feedback.lance/`, `symbols.lance/` — same, empty
- `auth/` — empty dir (credentials file destroyed)
...
4. Re-running `memento tenant delete` against a deleted token returns `AUTH_FAILED` — the tenant is unaddressable.
...
| `confirmation_required` | **PASS** | typing "no" or anything ≠ `yes` returns `INVALID_INPUT` (exit 2) |
| `non_existent_tenant` | **PASS** (by design) | no `--name` flag; token auth gates erase, so non-authenticated IDs are unreachable |
| `lance_old_versions_purged` | **PASS** | `Prune` step ran; only `latest_version_hint` and current `_versions/*` manifest remain — old data files gone (no fingerprint bytes) |
...
- `crates/memento-application/src/erase.rs:53-113` — `AppService::erase` (4-step chain: store purge → key destruction → code/config → audit)
- `crates/memento-lancedb/src/maintenance.rs:226-...` — `erase` (delete → Compact → Prune)
- `crates/memento-cli/src/commands/tenant.rs:109-137` — `tenant delete` ceremony + credential destruction
- `crates/memento-cli/src/commands/mod.rs:26-...` — `confirm_ceremony` (stdin `yes` check)
- `crates/memento-application/src/audit.rs:239-...` — `AuditLogger::erase` (T-120)
- Obs #2621 — "Opening the store trips the restore quiesce check" (same root cause: open LanceDB + Windows file locks)
...
- Test data: `F:\.memento-smoke\db\tenants\019fee80-4b91-7ae2-a4b6-65839314ee1a\` (left as-is for forensic follow-up; empty)
