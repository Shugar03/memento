# Memento RS — MCP `code.*` Tools Happy-Path Validation
...
**Project indexed**: `F:\OPENCODE proyectos\Memento RS` (13 crates, Memento RS MVP)
...
```powershell
.\target\release\memento.exe code index "F:\OPENCODE proyectos\Memento RS" --root "F:\.memento-smoke" --json
```
...
- **2.0 s** cold index on this host. Matches the documented 10k LOC < 2s target (the project is 11–12k LOC; the L1 OKF bundle serialization is the dominant cost).
...
|---|---|---|---|
| `code.project_overview` | `project_id` | — | Resumen arquitectónico del proyecto (capa L4). |
| `code.symbol_lookup` | `project_id`, `symbol` | — | Busca un símbolo (función, tipo, constante) en el índice. |
| `code.callers_of` | `project_id`, `symbol` | — | Quiénes llaman a un símbolo (hasta profundidad 2). |
| `code.callees_of` | `project_id`, `symbol` | — | A quién llama un símbolo (hasta profundidad 2). |
| `code.impact` | `project_id`, `symbol` | — | Alcance de impacto inverso: qué se rompería si cambia un símbolo. |
| `code.dependencies` | `project_id` | — | Dependencias del proyecto y detección de ciclos. |
| `code.search` | `project_id`, `query` | `limit: uint (default 0)` | Busca código por símbolo o texto (literal y semántico). |
| `code.graph_dump` | `project_id` | — | Grafo canónico `{nodos, aristas}` del proyecto. |
...
### 3.1 `code.project_overview` — 570.5 ms — PASS
...
- L2 (symbol hashmap) → 1.6 ms warm. Match for the spec target.

### 3.3 `code.callers_of` — 2.4–2.8 ms — PASS
...
- L3 → 2.4 ms. Returns up to depth-2 inverse call graph in human-readable form.
- Note: with `symbol=TenantContext` (a Struct) the response is `{symbols: []}` — correct, structs have no call-site relationships. The brief's `TenantContext` choice was unhelpful for L3 testing. The follow-up probe (`code_tools_e2e_function.json`) used `symbol=build` to actually exercise the L3 edges.

### 3.4 `code.callees_of` — 2.5–3.0 ms — PASS
...
### 3.5 `code.impact` — 2.1–3.1 ms — PASS
...
### 3.6 `code.dependencies` — 5.8 ms — PASS
...
{ "symbols": [
  ...
  … 200+ edges …,
  "cycle: modules/crates/memento-application/src -> modules/crates/memento-application/src/audit -> …"
]}
```

- L3 (canonical) → 5.8 ms. Returns the full module-level dependency graph AND surfaces detected cycles inline (so a client can stop, no follow-up call needed).
- Edges are `<module_a> -> <module_b>` strings. Cycles are tagged with the `cycle:` prefix. **The cycles match what `code.project_overview` reports** — same source, both consumers honest.

### 3.7 `code.search` — 5940 ms (5.94 s) — PASS-with-warning
...
{ "artifacts": [
  ...
  { "artifact_id": "search", "content": { … }, "kind": "symbol", "project_id": "cf66fd58222bacf9" },
  { "artifact_id": "search", "content": { … }, "kind": "symbol", "project_id": "cf66fd58222bacf9" },
  { "artifact_id": "search", "content": { … }, "kind": "symbol", "project_id": "cf66fd58222bacf9" },
  { "artifact_id": "export", "content": { … }, "kind": "symbol", "project_id": "cf66fd58222bacf9" }
]}
```

- 5 hits, each with full provenance (`id`, `kind`, `name`, `file`, `start_line`, `end_line`, `score`).
- Hybrid semantic+BM25 — the score is the only fusion signal surfaced (0.86–0.87 cluster).
- **Latency 5.94 s** is the cold path: first call loads the E5Base ONNX runtime + MultilingualE5Base tokenizer, builds the index, then embeds the query. Subsequent calls (the empty-query test, 45 ms — see §4) confirm warm path is fast.

### 3.8 `code.graph_dump` — 61.5 ms — PASS
...
- L3 (canonical) → 61.5 ms for the full graph. **Gephi/Cytoscape/Sigma compatible**: nodes have `id`, edges have `source`/`target`.
...
|---|---|---|---|---|
| `code.project_overview` | 570.5 ms | ✅ | L4 | includes cycle list inline |
| `code.symbol_lookup` | 1.6 ms | ✅ | L2 | — |
| `code.callers_of` | 2.4 ms | ✅ | L3 | empty for non-callable symbols (by design) |
| `code.callees_of` | 2.5 ms | ✅ | L3 | empty for non-callable symbols (by design) |
| `code.impact` | 2.1 ms | ✅ | L3 | empty for non-callable symbols (by design) |
| `code.dependencies` | 5.8 ms | ✅ | L3 | cycles surfaced inline |
| `code.search` | 5940 ms | ✅⚠ | L2+FTS+ANN | cold path; warm ~45 ms |
| `code.graph_dump` | 61.5 ms | ✅ | L3 | full graph; Gephi-compatible |
...
|---|---|---|---|---|---|
| 1 | `code.symbol_lookup` | `symbol="ThisSymbolDoesNotExist_zzzz"` | 2.5 ms | `{"symbol": null}` (is_error=false) | ⚠ Inconsistent — see B1 |
| 2 | `code.callers_of` | `symbol=""` | 1.5 ms | `{"symbols": []}` (is_error=false) | ⚠ Inconsistent — see B1 |
| 3 | `code.callees_of` | `symbol=12345` (int) | 1.0 ms | `failed to deserialize parameters: invalid type: integer 12345, expected a string` (is_error=true) | ✅ Correct |
| 4 | `code.impact` | unknown project | 1.3 ms | `{"code":"NOT_FOUND","detail":"code index for project '00000000-…' - run memento code index <path> first not found","exit_code":20,"message":"No encontrado.","message_en":"Not found.","message_es":"No encontrado."}` (is_error=true) | ✅ Correct, bilingual |
| 5 | `code.dependencies` | unknown project | 1.6 ms | same NOT_FOUND bilingual shape | ✅ Correct, bilingual |
| 6 | `code.search` | `query=""` | 45.5 ms | returns 5 hits (is_error=false) | ❌ Bug — see B2 |
| 7 | `code.graph_dump` | unknown project | 1.0 ms | NOT_FOUND bilingual | ✅ Correct, bilingual |
| 8 | `code.project_overview` | unknown project | 0.6 ms | NOT_FOUND bilingual | ✅ Correct, bilingual |
| 9 | `code.symbol_lookup` | `{}` (no fields) | 1.2 ms | `failed to deserialize parameters: missing field 'project_id'` (is_error=true) | ✅ Correct |
| 10 | `code.symbol_lookup` | original symbol after 8 errors | 1.1 ms | resolves normally | ✅ Session survives (REQ-MS-005) |

---

## 5. Bugs found

### B1 — Inconsistent "not found" shape across `code.*` tools

- `code.symbol_lookup("unknown")` → `{"symbol": null}` (is_error=false). Soft result.
- `code.callers_of("")` → `{"symbols": []}` (is_error=false). Soft result.
- `code.impact(unknown_project)` → NOT_FOUND bilingual error (is_error=true). Hard error.
...
Severity: **medium** — confusing for clients; no data loss.

### B2 — `code.search` accepts empty query and returns hits

```bash
code.search(project_id, query="", limit=5) → 5 hits
```

Per the `memory.search` precedent (REQ-MR-003), an empty query is an `INVALID_INPUT` precondition violation. Here, an empty query is treated as a generic no-constraint query and the L2+FTS+ANN pipeline returns whatever scores highest on the "no constraint" baseline (5 module-level artifacts with score 0.86). This is functionally a "list the top symbols of the project" side effect — undocumented behavior.

Severity: **medium** — the response is not what a caller would expect, and the side effect is undocumented. Worse on a tenant with sensitive code: it leaks the highest-scoring symbols of any indexed project.
...
### B3 — `code.search` cold-path latency 5.9 s

- First call: 5.94 s (embedder + index warm-up + query embed).
- Subsequent call (45 ms — see `search_empty_query`): warm path is fast.
...
Severity: **low** — single cold start; not a regression.
...
|---|---|---|---|
| `code.graph_dump` | `{nodes, edges}` | `{nodes: [1285], edges: [1270]}` | ✅ Gephi/Cytoscape/Sigma compatible |
| `code.search` | `[{chunk_id, text, score}]` | `{artifacts: [{artifact_id, content{file,start_line,end_line,signature,...}, score, kind, project_id}]}` | ⚠ No `chunk_id`; uses `artifact_id` + `content.id`. Compatible with the L1/L2 naming. |
| `code.project_overview` | L4 summary | Markdown text in `summary` field + `artifact_count` + `project_id` | ✅ |
| All symbol-bearing tools | full provenance | `id, kind, name, file, start_line, end_line, is_public, signature` | ✅ |
...
- Toolchain env (PowerShell, every shell):
  ...
  $env:MEMENTO_TOKEN = "memo_019fee0a-7f1a-78e2-b9a6-549fa58902c6_4K7qrU7j8vH6J6yBI7DAtDrwcYnKCFldWYyab5wE1x6UkgOl"
...
- The `code.*` family is a successful validation of the L1→L2→L3→L4 layered architecture on a real workspace.
